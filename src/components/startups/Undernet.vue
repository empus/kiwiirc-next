    ...
    <div class="kiwi-welcome">
        ...
        <form v-on:submit.prevent="startUp" class="u-form kiwi-welcome-form">
            <template v-if="server_type === 'default'">
                ...
                <label>
                    <span></span><input type="checkbox" v-model="show_services_box" /> I want to login to X
                </label>
                <template v-if="show_services_box">
                    <label>
                        <span>Username</span>
                        <input type="text" v-model="services.username" />
                    </label>
                    <label>
                        <span>Password</span>
                        <input type="password" v-model="services.password" />
                    </label>
                    <label class="kiwi-welcome-use-otp">
                        <span></span><input type="checkbox" v-model="show_services_otp_box" /> Use 2FA
                    </label>
                    <label>
                        <span></span><input type="checkbox" v-model="services.login_required" /> Hide host
                    </label>

                    <template v-if="show_services_otp_box">
                        <label>
                            <span>TOTP Token</span>
                            <input type="text" v-model="services.token" />
                        </label>
                    </template>
               </template>
...
export default {
    data: function data() {
        return {
            ...
            show_services_box: false,
            show_services_otp_box: false,
        ...
        startUp: function startUp() {
            ...
            if (this.server_type === 'znc') {
                ...
                if (this.services.username) {
                    netOptions.services = {
                        userpass: this.services.password,
                        username: this.services.username,
                        token: this.services.token,
                        login_required: this.services.login_required,
                        hide_hostmask: this.services.hide_hostmask,
                    };
                }
                net = state.addNetwork('Network', this.nick, netOptions);
            }
